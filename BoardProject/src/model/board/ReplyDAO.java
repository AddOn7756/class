...
public class ReplyDAO {
	...
	// 댓글 업데이트 
	static String sql_UPDATE = "UPDATE BOARD_REPLY SET R_CONTENT=? WHERE R_NUM=?";
	// 유저 ID 값 댓글 전체출력
	static String sql_SELECT_ALL_USER = "SELECT R_ID, B_ID, USER_NUM, "
			...
			+ "USER_NUM=? AND ROWNUM <=? ORDER BY R_DATE DESC) BOARD_REPLY WHERE ?<=RNUM";
	// 댓글 전체출력
	static String sql_SELECT_ALL = "SELECT R_ID, B_ID, USER_NUM, "
			...
			+ "WHERE ROWNUM <=5 ORDER BY R_DATE DESC) BOARD_REPLY WHERE 2<=RNUM";
	...
	public ArrayList<ReplySet> getDBList(int userNum, int pageNum, int bId) {
		...
		try {
			...
			while(rs.next()) {
				...
				if (rvo.getParentId() > 0) {
					String rrsql = "SELECT * FROM BOARD_REPLY WHERE PARENT_ID=? ORDER BY B_DATE DESC";
